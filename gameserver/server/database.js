...
exports.validateOneTimeToken = function(callback) {
    //assert(token);

   query('WITH t as (UPDATE sessions SET expired = now() WHERE id = 82 AND ott = TRUE RETURNING *)' +
            'SELECT * FROM users WHERE id = 82',
         function(err, result) {
            if (err) return callback(err);
                ...
                client.query('SELECT * FROM referral where user_id = $1', [username], function(err, res) {
                    ...
                    if(res.rows.length>0){
                        ...
                        client.query('SELECT * FROM users WHERE id = $1', [referred_by], function(err, result) {
                            ...
                            let amt = (amount*rate)/100;
                            client.query('INSERT INTO referral_earnings(user_id,user_id_benefit,amount,created_on,user_username) values($1,$2,$3,$4,$5)', 
