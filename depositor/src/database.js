...
var pg = require('pg');
const ethClient = require('./ether_client');
...
exports.getLastBlock = function(callback) {
    query('SELECT * FROM blocks ORDER BY height DESC LIMIT 1', function(err, results) {
        ...
        if (results.rows.length === 0) {
            return ethClient.getBlock(0).then(block => {
                return callback(null, { height: 0, hash: block.hash }); // genesis block
            });
        }
             // genesis block
...
exports.addDeposit = function(userId, txid, amount, callback) {
    ...
    getClient(function(client, callback) {
        async.parallel([
            function(callback) {
                 client.query('INSERT INTO fundings(user_id, amount, ethereum_deposit_txid, description) ' +
                    "VALUES($1, $2, $3, 'Ethereum Deposit')",
