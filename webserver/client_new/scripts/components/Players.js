        ...
        render: function() {
            ...
            if(game.gameState === 'IN_PROGRESS' || game.gameState === 'STARTING') {
                ...
                trUsersLostPlaying = usersLostPlaying.map(function(entry, i) {
                    ...
                    if (bonus) {
                        //profit = Clib.formatSatoshis((profit + bonus)*100);
                    ...
                    return D.tr({ className: classes, key: 'user' + i },
                        D.td(null, D.a({ href: '/user/' + entry.username,
                            ...
                            entry.username)),
                        D.td(null, '-'),
                        D.td({id:"bet_"+entry.username}, Clib.formatBits(entry.bet)),
                        D.td(null, '-'),
                        D.td({id:"profit_"+entry.username}, profit)
                    );
                });
                ...
                trUsersWonCashed = usersWonCashed.map(function(entry, i) {
                    ...
                    if (bonus) {
                        //profit = Clib.formatBits(profit + bonus);
                        profit = Clib.formatBits(profit);
                        bonus = '0%'
                    } else {
                        profit = Clib.formatBits(profit);
                        bonus = '-';
                    ...
                    return D.tr(
                        ...
                        D.td({id:"bet_"+entry.username}, Clib.formatBits(bet)),
                        D.td(null, '-'),
                        D.td({id:"profit_"+entry.username}, profit)
                    );
                });
