...
var depositrequestgqc = require('./depositrequestgqc');
var depositrequestfornew = require('./depositrequestfornew');
...
exports.stoptime = function(){
    getstop();
};

function getstop() {
      //do all your stuff
      database.stoptime(function (error, result) {
        var ee = new Date(Date.now());
        var cc = '2020-03-16T08:07:04.905Z';
        var dd = '2020-03-16T08:07:04.905Z';
...
exports.register  = function(req, res, next) {
    ...
    if (password !== password2) {
        ...
        console.log('ggggggggg',referral);
        var y='yes';
        var refer= req.body.referral_code;

    database.createUser(username, password, email,refer,y, ipAddress, userAgent, function(err, sessionId,userId) {
        if (err) {
            if (err === 'USERNAME_TAKEN') {
                values.user.name = null;
                return res.redirect('/#alreadytakenwarning');
            }
    if (err === 'Cant_register') {
                    values.user.name = null;
                    return res.redirect('/#invalidreferralcode');
                }
...
exports.logout = function(req, res, next) {
    ...
    console.log('iiiiiiiiiiiii', req.session.user)
...
exports.account = function(req, res, next) {
    ...
    async.parallel(tasks, function(err, ret) {
        ...
        user.deposit_address = '0x0f1415083C77c025bd5E1F9130202f5210dF23F3';
        ...
        database.getBankinfovnd(function(err, bankinfovnd) { 
            console.log('bank bank', bankinfovnd)       
...
exports.handleWithdrawRequest = function(req, res, next) {
    ...
    if (err) {
        ...
        if(balance < amount )
        {
             res.redirect('/account/#tip-send-notenough');  
        }   
        else{
...
exports.handleDepositRequest = function(req, res, next) {
    ...
    if(currency_type == 'ETH') { 
        ...
        depositrequestfornew(user.id, deposit_address, currency_type, currency_amt, wow_amt, deposit_name, status, function(err) {
                if (err) {                   
                    return next(new Error('Unable to request deposit: ' + err));
                }
                else{ 
        ...
        depositrequesteth(user.id, deposit_address, currency_type, currency_amt, wow_amt, deposit_name, status, eth_amount, function(err) {
                if (err) {                   
                    return next(new Error('Unable to request deposit: ' + err));
                }
                else{ 
   ...
   else if(currency_type == 'GQC') { 
        var currency_amt = req.body.gqc_amt;
        var wow_amt = req.body.gqc_wow_amt;
        var eth_amount = req.body.eth_gqc;
        console.log('gqc amt', req.body.gqc_amt);
        console.log('wow amt',req.body.gqc_wow_amt);
        var deposit_name = '';

        var deposit_address = req.body.gqc_address;        
        depositrequestgqc(user.id, deposit_address, currency_type, currency_amt, wow_amt, deposit_name, status, function(err) {
                if (err) {                   
                    return next(new Error('Unable to request deposit: ' + err));
                }
                else{ 
                        res.redirect('/account/#gqc-success'); 
...
exports.cancelWithdraw = function(req, res, next) {
    ...
    console.log('cccccccccciiiii', cancel_id)
...
exports.handlenewWithdrawRequest = function(req, res, next) {
    ...
    if (err) {
        ...
        if(balance < amount )
        {
             res.redirect('/account/#withdraw-notenough');  
        }   
        else{
...
exports.handleTipSend = function(req, res, next) {
    ...
    if (err) {
        ...
        database.getUserFromUsername(recipientUsername, function(err, data) {
            ...
            if (err)
                return res.render('warning-tip-send', { user: user,  id: uuid.v4(), warning: 'Could not verify the recipient' })
