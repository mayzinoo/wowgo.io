...
//Returns a sessionId
exports.createUser = function(username, password, email,refer,y, ipAddress, userAgent, callback) {
    assert(username && password && refer && y);
    ...
    getClient(
        function(client, callback) {
            ...
            client.query('SELECT COUNT(*) count FROM users WHERE lower(username) = lower($1) AND referral =$2', [refer,y],
                                function(err, data) {
            ...
            });/*end checking referral code exist or not*/
...
exports.addReferral = function(username, referredBy, callback) {
    ...
    query('SELECT * FROM users where username = $1', [referredBy], function(err, res) {
        ...
        query('INSERT INTO referral( user_id, referred_by,created_on) VALUES($1, $2, $3)', [username, res.rows[0]['id'],date], function(err, res) {
            ...
            console.log(res)
            console.log('referral');
            if(err) return callback('eeeeeeeeeeeeeeee', err);
...
exports.validateUser = function(username, password, otp, callback) {
    assert(username && password);
    query('SELECT id, username, password, mfa_secret,status FROM users WHERE lower(username) = lower($1)', [username], function (err, data) {
        if (err) return callback(err);
        ...
        if (!verified)
        {
            ...
            callback(null, user.id, user.username, user.password);
        }   
...
exports.expireSessionsByUserId = function(userId, callback) {
    ...
    query('UPDATE sessions SET expired = now() WHERE user_id = $1 AND expired > now()', [userId], function(err, res) {        
        callback(null);
    });
};

exports.addSession = function(userId, ipAddress, userAgent, remember, callback) {
    ...
    console.log('exxxxxxxxxxxxx', expired);

    query('INSERT INTO sessions(id, user_id, ip_address, user_agent, expired) VALUES($1, $2, $3, $4, $5) RETURNING id',
...
function createSession(client, userId, ipAddress, userAgent, remember, callback) {
    ...
    console.log('exxxxxxxxxxxxxxxxxxx', expired)
    ...
    client.query('INSERT INTO sessions(id, user_id, ip_address, user_agent, expired) VALUES($1, $2, $3, $4, $5) RETURNING id',
        ...
        var session = res.rows[0];
        assert(session.id);
        console.log('db db db', session.id)
...
exports.getSession = function(userId, callback) {
    ...
    query('SELECT * FROM sessions WHERE user_id = $1 order by created desc limit 1', [userId], function(err, response) {
        ...
        var sss = data[0];       
        //console.log('sssssssssss', sss.id)
        callback(null, sss.id);
    });
};
...
exports.requestdepositeth = function(userid, deposit_address, currency_type, currency_amt, wow_amt, deposit_name, status, eth_amount, callback) {
    
    assert(userid && callback);

    var sql = 'INSERT INTO fundings (user_id, ethereum_deposit_address, currency_type, currency_amount, amount, owner_name, status, eth_amount ) values($1, $2, $3, $4, $5, $6, $7 ,$8)'; 

    query(sql, [userid, deposit_address, currency_type, currency_amt, wow_amt, deposit_name, status, eth_amount ], function(err, res) {
        // if(err) 
        //     console.log(err); 
        //     return callback(err);
...
exports.requestdepositgqc = function(userid, deposit_address, currency_type, currency_amt, wow_amt, deposit_name, status, callback) {
    assert(userid && callback);
...
exports.requestdepositfornew = function(userid, deposit_address, currency_type, currency_amt, wow_amt, deposit_name, status, callback) {    
    assert(userid && callback);
    var sql = 'INSERT INTO fundings (user_id, ethereum_deposit_address, currency_type, currency_amount, amount, owner_name, status ) values($1, $2, $3, $4, $5, $6, $7)'; 
