...
exports.addReferral = function(username, referredBy, callback) {
    var date = new Date();
    query('SELECT * FROM users where LOWER(username) = LOWER($1)', [referredBy], function(err, res) {
        console.log(err);
        console.log(res)
...
exports.getethaddress=function(callback){
        query('SELECT id,address FROM ethaddress  where istake=0 order by id desc limit 1', function(err, data) {
        if(err) return callback(err);
...
exports.validateUser = function(username, password, otp, callback) {
    ...
    query('SELECT id, password, mfa_secret,status FROM users WHERE lower(username) = lower($1)', [username], function (err, data) {
        if (err) return callback(err);
...
exports.updatememberbalance = function(senderid, receiverid, balance,  callback) {    
    ...
    var sql2 = 'UPDATE users SET balance = balance + $1 WHERE id = $2';    
...
//     assert(userid && callback);
//     var sql = 'INSERT INTO tdepositrequest (userid, currency_type, currency_amount, wow_amount, depositname, deposit_address) values($1, $2, $3, $4, $5, $6)'; 
